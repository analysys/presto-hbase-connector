...
public class HBaseRecordSet implements RecordSet {
        ...
        try (Table table = connection
            ...
            // client side region scanner
                ...
                try {
                    ...
                    log.error(abce, "E-3-1 AlreadyBeingCreatedException: " + abce.getMessage());
                    ...
                    log.error(re, "E-3-2 RemoteException: " + re.getMessage());
                    scanner = createClientSideRegionScannerWithExceptionHandle(conf, fs, root, htd, regionInfos.get(hBaseSplit.getRegionIndex()), scan);
                } catch (Exception e) {
                    log.error(e, "E-3-3 Exception: " + e.getMessage());
                    scanner = createClientSideRegionScannerWithExceptionHandle(conf, fs, root, htd, regionInfos.get(hBaseSplit.getRegionIndex()), scan);
                }
                if (scanner == null) {
    ...
    private ClientSideRegionScanner createClientSideRegionScannerWithExceptionHandle(
        ...
        try {
            ...
            log.error(e, "E-3-4 Exceptionnnnnnnnnnnnnnnnnn -> Create ClientSideRegionScanner failed! Track is : " + e.getMessage());
            return null;
        }
    }
